...
## 別ツール: マンスリー物件 検索ランチャー
同じサーバで `http://100.67.251.19:5055/monthly.html` も配信（トップからリンクあり）。
こちらは**レオパレス等の商用マンスリーマンションをポータル横断で探す**ランチャーで、
都道府県を選ぶと各ポータル（レオパレス21／LIFULL HOME'S／マンスリーマンション.com／
グッドマンスリー／マンスリー48）の最新の検索結果を新規タブで開きます。本体の
「独立系・直予約ゲストハウスDB」とは別目的の独立ページです。
